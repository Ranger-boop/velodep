...
{
    RSF_tau_fix::set_ini_vals(mu_0, v_0, disp_0);
    this->k_s = k_s;
}
...
)
{
    if (num_vlp != num_t) {
        ...
        RSF_tau_fix::set_indpt_val(
            sig_n, num_sig_n,
            pp, num_pp,
            t, num_t
        );
        this->vlp = new std::valarray<double>(vlp, num_vlp);
        this->vlp_evl = vlp[0];
    }
}

RSF_tau_chg::RSF_tau_chg() : RSF_tau_fix()
{
    this->k_s = 0;
    this->vlp = nullptr;
    this->vlp_evl = 0;
...
) : RSF_tau_fix(a, b, Dc, alpha, mu_0, v_0, disp_0)
{
    this->k_s = k_s;
    this->vlp = nullptr;
    this->vlp_evl = 0;
...
RSF_tau_chg::RSF_tau_chg(const RSF_tau_chg& rsf) : RSF_tau_fix(rsf)
{
    this->k_s = rsf.k_s;
    this->vlp = new std::valarray<double>(*rsf.vlp);
...
double RSF_tau_chg::dTau_dt(double disp_, double theta_, double tau_) const
{
...
void RSF_tau_chg::solve_rk4()
{
    ...
    } else if (!this->vlp || !this->sig_n || !this->pp || !this->t) {
